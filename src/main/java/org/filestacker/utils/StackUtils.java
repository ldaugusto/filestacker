...
public final class StackUtils {
	 ...
	 * 
	 * Método original de FastRandomIO e FastOutput, nas tablets 0.x
	 * 
	 * @param text
	 ...
	 * aquelas originadas por strToBytes. Caso contrário, é possível que, em
	 * alguns casos, este método falhe.
	 * 
	 * Método original de FastRandomIO e FastInput, nas tablets 0.x
	 * 
	 * @param bytearr
	 ...
	 * Utilidade da criação é a utilização para visualizar melhor o MD5. Mas
	 ...
	 * origem) é convencionado em função do segundo.
	 * 
	 * Convenção sobre customização FTW!!1!
	 * 
	 * @param file_out
	 * @throws IOException
	 */
	 ...
	 * 
	 * É uma função bem simples, onde a única peculiaridade é o parâmetro
	 * <code>long</code>, que é o valor adicionado ao final do array de int
	 * retornado. Útil para a conversão de dados lidos de um arquivo em índice
	 * de uma tablet.
	 * 
	 * @param bytearr
	 ...
	 * @param path
	 *            O path onde estarão as tablets
	 * @return A referencia para onde será criado o arquivo de tablet.
	 */
	public static File generateStackFile(final int first, final String path) {
		...
		if (!pathFile.exists()) {
			...
			if (!created) logger
					.debug("Não foi possível criar estrutura de diretórios para "
	 ...
	 * 
	 * A grande velocidade se deve a utilização dos métodos de Java NIO, que
	 * fazem essa operação em baixo nível e não bloqueante.<br>
	 * 
	 * Por padrão, esse método será utilizado na construção das tablets, onde a
	 * coleção é escrita num arquivo temporário que ao final é 'colado' ao final
	 * do arquivo da tablet, nos métodos populate() e writeTablet().
	 * 
	 * @param file_in
	 ...
	 *            O array a ser convertido
	 * @param size
	 *            O valor a ser colocado na última posição
	 * @return <code>int[]</code> O array já convertido
	 */
	public static int[] byteArrayToIntArray(byte[] bytearr, long size) {
		// +1 para colocar no final a posição final do arquivo de tablet, útil
		...
		// Anexa a posição final do documento
	 ...
	 * 
	 * Se estiverem em diferentes partições, o 'move' falha, e então se torna um
	 * copy + delete
	 * 
	 ...
	 * @return o tempo gasto na operação
	 * @throws IOException
	 */
	...
	public static long moveDirectoryContents(File fromPath, File toPath)
			throws IOException {
		/*
		 * TODO É possível melhorar este método. Se o File.renameTo é possível,
		 * então o ideal é mover logo os sub-diretórios 'raiz', ao invés de
		 * realizar múltiplos mkdirs() + moves individuais. Entretanto essa
		 * otimização é inferior a obtida com este método atual, então só
		...
		for (File f : fileList) {
			...
			if (!created && logger.isDebugEnabled()) logger
					.debug("Não foi possível criar os diretórios para "
	 ...
	 * @param startDir
	 *            De onde se quer considerar o começo
